    ...
    const stat = {
        ...
        upgrade: Character.stat.upgrade.map(w => {
            return w.props.map(prop => {
                return ['원소 마스터리', '기초 HP', '기초 공격력', '기초 방어력'].includes(prop.type) ? prop.value : (Math.round(prop.value * 1000) / 10) + '%'
            })
        })
    }
...
for (const Weapon of Weapons) {
    ...
    const weapon = {
        ...
        icon: Weapon.icon,
        baseAtk: statTransform(Weapon.stat[0]) + ' ~ ' + statTransform({
            type: Weapon.stat[0].type,
            value: Weapon.stat[0].value * curves.weapons[lvlMap[maxasc]][Weapon.stat[0].curve] + Weapon.promote[maxasc - 1].props[0].value
        }),
        substat: Weapon.stat[1] ? {
            name: Weapon.stat[1].type,
            value: statTransform(Weapon.stat[1]) + ' ~ ' + statTransform({
